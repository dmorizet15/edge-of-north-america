...
export default function DayChapter({ day }: { day: Day }) {
  return (
    ...
    >
      {/* Cinematic photo band with the day's title over it */}
      {day.photo ? (
        <div className="relative h-[52vh] min-h-[380px] w-full overflow-hidden">
          <EditorialImage photo={PHOTOS[day.photo]} fill drift />
            ...
            <Reveal delay={0.1}>
              <div className="rounded-sm border border-paper/10 bg-black/20 p-6">
                ...
                <dl className="mt-6 divide-y divide-paper/10">
                  ...
                  <Detail label="Wildlife" value={day.wildlife} />
...
function StopMoment({ stop }: { stop: Stop }) {
  return (
    <Reveal>
      <li className="relative grid grid-cols-[7rem_1fr] gap-5 pb-8">
        <span
